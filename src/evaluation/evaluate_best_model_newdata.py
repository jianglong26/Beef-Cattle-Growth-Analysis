...
class BestModelEvaluator:
    ...
    def load_test_dataset(self, dataset_path: str = None, dataset_yaml_path: str = None) -> Tuple[List[Path], List[Path]]:
        """加载测试数据集
        
        支持两种方式：
        1. 直接指定数据集目录（包含images和labels子目录）
        2. 通过YAML配置文件（包含test/images和test/labels）
        
        Args:
            dataset_path: 数据集根目录路径（直接包含images和labels文件夹）
        ...
        if dataset_path:
            ...
            # 直接在根目录下查找images和labels
            test_images_dir = dataset_root / 'images'
            test_labels_dir = dataset_root / 'labels'
            
            if not test_images_dir.exists():
                raise FileNotFoundError(f"Images directory not found: {test_images_dir}")
            if not test_labels_dir.exists():
                raise FileNotFoundError(f"Labels directory not found: {test_labels_dir}")
                
            self.logger.info(f"Loading dataset from: {dataset_root}")
            
        # 方式2: 通过YAML配置文件
        elif dataset_yaml_path:
            ...
            self.logger.info(f"Loading test set from YAML config: {dataset_yaml_path}")
        else:
            raise ValueError("Either dataset_path or dataset_yaml_path must be provided")
        ...
        self.logger.info(f"✓ Found {len(valid_label_paths)} images with labels")
        self.logger.info(f"✓ Skipped {len(image_paths) - len(valid_label_paths)} images without labels")
    ...
    def calculate_ap(self, predictions: List[Dict], ground_truths: List[Dict], 
        ...
        for pred_idx, pred in enumerate(predictions):
            ...
            for gt_idx, gt in gt_by_class[pred_class]:
    ...
    def calculate_map_at_iou_range(self, all_predictions: List[List[Dict]], 
        ...
        Returns:
            Dict: 包含各种mAP指标
        """
        self.logger.info("Flattening predictions and ground truths...")
        # 展平所有预测和标签
        flat_predictions = []
        flat_ground_truths = []
        ...
        self.logger.info(f"Total predictions: {len(flat_predictions)}, Total ground truths: {len(flat_ground_truths)}")
        ...
        self.logger.info("Computing AP at different IoU thresholds...")
        for idx, iou_thresh in enumerate(iou_thresholds):
            self.logger.info(f"  [{idx+1}/10] Computing AP @ IoU={iou_thresh:.2f}...")
    ...
    def evaluate_on_test_set(self, dataset_yaml_path: str = None, dataset_path: str = None,
                             conf_threshold: float = 0.25, batch_size: int = 16, 
                             save_visualizations: bool = True, max_vis_samples: int = 50):
        """在held-out测试集上评估最佳模型
        
        Args:
            dataset_yaml_path: 数据集配置文件路径（用于已划分数据集）
            dataset_path: 数据集目录路径（用于未划分数据集）
        ...
        # 加载测试数据集
        image_paths, label_paths = self.load_test_dataset(
        ...
        all_predictions = []
        all_ground_truths = []
        vis_count = 0  # 统计保存的可视化数量
        
        self.logger.info(f"Processing {len(image_paths)} test images...")
        self.logger.info(f"Will save up to {max_vis_samples} visualization samples" if save_visualizations else "Visualization disabled")
        ...
        for batch_start in range(0, len(image_paths), batch_size):
            ...
            for i, (img_path, label_path, result) in enumerate(zip(batch_paths, batch_labels, batch_results)):
                ...
                if save_visualizations and (batch_start + i) < max_vis_samples:
                    # 为可视化时应用置信度阈值过滤
                    vis_predictions = [p for p in predictions if p['confidence'] >= conf_threshold]
                    vis_image = self.draw_detections(image, vis_predictions, ground_truths)
                    vis_path = vis_dir / f"{img_path.stem}_result.jpg"
                    cv2.imwrite(str(vis_path), vis_image, [cv2.IMWRITE_JPEG_QUALITY, 95])
                    vis_count += 1
        
        if save_visualizations:
            self.logger.info(f"Saved {vis_count} visualization images to: {vis_dir}")
        
        # 统计信息
        self.logger.info(f"\n{'='*80}")
        self.logger.info(f"PROCESSING SUMMARY:")
        self.logger.info(f"{'='*80}")
        self.logger.info(f"Total images processed: {len(all_predictions)}")
        self.logger.info(f"Total ground truth boxes: {sum(len(gts) for gts in all_ground_truths)}")
        total_preds = sum(len(preds) for preds in all_predictions)
        filtered_preds = sum(len([p for p in preds if p['confidence'] >= conf_threshold]) for preds in all_predictions)
        self.logger.info(f"Total predictions (all confidences): {total_preds}")
        self.logger.info(f"Filtered predictions (conf >= {conf_threshold}): {filtered_preds}")
        self.logger.info(f"{'='*80}\n")
        ...
        # 计算评估指标（使用指定的置信度阈值）
        self.logger.info(f"\nCalculating evaluation metrics at confidence threshold = {conf_threshold}...")
        
        # 过滤预测结果（应用置信度阈值）
        filtered_predictions = []
        for preds in all_predictions:
            filtered = [p for p in preds if p['confidence'] >= conf_threshold]
            filtered_predictions.append(filtered)
        
        metrics = self.calculate_map_at_iou_range(filtered_predictions, all_ground_truths)
    ...
    def analyze_confidence_thresholds(self, all_predictions: List[List[Dict]], 
        ...
        Returns:
            Dict: 不同阈值下的性能指标
        """
        self.logger.info("\nAnalyzing confidence thresholds (this may take a few minutes)...")
        
        # 测试的置信度阈值范围（减少测试点以提高速度）
        conf_thresholds = np.arange(0.1, 1.0, 0.05)
        results_by_conf = []
        
        for idx, conf_thresh in enumerate(conf_thresholds):
            self.logger.info(f"  [{idx+1}/{len(conf_thresholds)}] Testing confidence threshold = {conf_thresh:.2f}...")
        ...
        self.logger.info("Confidence threshold analysis completed!")
...
def main():
    ...
    parser.add_argument('--data', type=str, default=None,
                       help='Dataset YAML file (for split datasets with test/ folder)')
    
    parser.add_argument('--dataset', type=str, default="data/yolo_det_dataset_total_1997",
                       help='Dataset directory (for unsplit datasets with images/ and labels/ folders)')
    parser.add_argument('--output', type=str, default="YOLOV11/output/yolov11s_cattle_20260109_223121_500/test_results_1997",
    ...
    if not args.data and not args.dataset:
        parser.error("Either --data (YAML config) or --dataset (directory path) must be provided")
