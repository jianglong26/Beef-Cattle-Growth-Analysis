...
TRAINING_CONFIG = {
    "epochs": 200,
    "batch_size": 32,  # ↑ 增大batch提高稳定性（16→24）
    "learning_rate": 0.0005,  # ↓ 降低学习率，更适合小数据集（0.001→0.0005）
    "momentum": 0.937,
    "weight_decay": 5e-3,
    "warmup_epochs": 15,  # ↑ 延长warmup，减少前期震荡（5→15）
...
AUGMENTATION_CONFIG = {
    ...
    # 几何增强（最小化，保持俯瞰视角）
    "degrees": 3.0,   # ↓ 降低旋转角度（5.0→3.0）俯瞰视角下不需要大旋转
    "translate": 0.08,# ↓ 降低平移（0.1→0.08）牛群位置相对稳定
    "scale": 0.2,     # ↓ 降低缩放（0.3→0.2）同一高度拍摄，牛大小变化小
    ...
    # 高级增强（降低强度或关闭）
    "mosaic": 0.5,    # ↓ 降低mosaic概率（1.0→0.5）过强会破坏场景一致性
